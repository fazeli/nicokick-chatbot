...
router.post("/faqs", async (req, res) => {
    try {
        ...
        // Validate FAQ data
        const validatedFaq = schema_1.insertFaqSchema.safeParse(faqData);
        ...
        await updateFaqDataFile([...await storage_1.storage.getFaqs(), {
                ...validatedFaq.data,
                id: await getNextFaqId()
            }]);
        // Generate embedding for the new FAQ
        const newFaq = (await storage_1.storage.getFaqs()).find(f => f.question === validatedFaq.data.question &&
        ...
        if (newFaq) {
            try {
                ...
                await storage_1.storage.updateFaqEmbedding(newFaq.id, embedding);
            }
            catch (error) {
...
router.put("/faqs/:id", async (req, res) => {
    try {
        ...
        if (updatedFaq.question !== faqs[faqIndex].question ||
            ...
            try {
                ...
                const embedding = await embeddingService_1.embeddingService.generateEmbedding(textToEmbed);
                await storage_1.storage.updateFaqEmbedding(id, embedding);
...
async function updateFaqDataFile(faqs) {
    // ESM modules don't have __dirname, so we need to construct paths differently
    // We'll use import.meta.url instead
    const currentFilePath = new URL(import.meta.url).pathname;
    const baseDir = path_1.default.dirname(path_1.default.dirname(currentFilePath)); // Go up two levels to get to server/
    const faqsFilePath = path_1.default.join(baseDir, "data/faq.ts");
    // Create a backup first
    const backupPath = path_1.default.join(baseDir, "data/faq.backup.ts");
    if (fs_1.default.existsSync(faqsFilePath)) {
        fs_1.default.copyFileSync(faqsFilePath, backupPath);
    }
    try {
...
`;
        // Write to file
        fs_1.default.writeFileSync(faqsFilePath, faqsContent);
        ...
        try {
            ...
            await storage_1.storage.initializeFaqs(simplifiedFaqs);
        ...
        if (fs_1.default.existsSync(backupPath)) {
            fs_1.default.copyFileSync(backupPath, faqsFilePath);
        }
        throw error;
    }
    finally {
